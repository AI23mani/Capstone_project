...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            data_rm= request.POST.get('radius_mean')
            data_cm=request.POST.get('concavity_mean')
            data_rw=request.POST.get('radius_worst')
            data_csw=request.POST.get('compactness_worst')
            data_cw=request.POST.get('concavity_worst')
            #print (data)
            ...
            data = np.array([data_rm,data_cm,data_rw,data_csw,data_cw])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_rm':data_rm,'data_cm':data_cm,'data_rw':data_rw,'data_csw':data_csw,'data_cw':data_cw,
                                                        'out':out})
